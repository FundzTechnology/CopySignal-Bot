...
class AuthHandler {
    ...
    setToken(token) {
        this.token = token;
        (0, utils_js_1.setToLocalStorage)("cocobase-token", token);
    ...
    setUser(user) {
        this.user = user;
        (0, utils_js_1.setToLocalStorage)("cocobase-user", JSON.stringify(user));
    }
    /**
     ...
     */
    async initAuth() {
        const token = (0, utils_js_1.getFromLocalStorage)("cocobase-token");
        const user = (0, utils_js_1.getFromLocalStorage)("cocobase-user");
        if (token) {
            this.token = token;
            if (user) {
     ...
     */
    send2FACode(email) {
        return this.request("POST", `/auth-collections/2fa/send-code`, {
            email,
        }, false);
    }
    /**
