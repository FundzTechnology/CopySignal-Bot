...
export function containsTriggerKeyword(rawText, triggerKeyword) {
...
export function scoreSignal(parsed, channelRules) {
    let score = 0;
    ...
    if (parsed.stop_loss && parsed.entry) {
        const slOnWrongSide = ((parsed.side === 'Buy' && parsed.stop_loss >= parsed.entry) ||
    ...
    if (parsed.take_profits.length > 0 && parsed.entry) {
        ...
        const tpOnWrongSide = ((parsed.side === 'Buy' && primaryTP <= parsed.entry) ||
