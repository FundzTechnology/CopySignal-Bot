...
if (bot) {
    bot.onText(/\/start(.*)/, async (msg, match) => {
        ...
        if (!param) {
            ...
            await bot.sendMessage(chatId, `👋 *Welcome to CopySignal Bot!*\n\n` +
                ...
                `Once linked, you'll get instant notifications for every trade.`, { parse_mode: 'Markdown' });
            return;
        }
        ...
        try {
            ...
            await bot.sendMessage(chatId, `✅ *Account linked successfully!*\n\n` +
                ...
                `/help — See all commands`, { parse_mode: 'Markdown' });
            ...
            await bot.sendMessage(chatId, `❌ Failed to link your account. Please check your link and try again, or contact support.`);
        }
    });
    // ── /status Command ─────────────────────────────────────────────────────────
    bot.onText(/\/status/, async (msg) => {
        ...
        await bot.sendMessage(chatId, `🟢 *Bot Status: Online*\n\n` +
            ...
            `make sure your Telegram is linked in Dashboard → Settings.`, { parse_mode: 'Markdown' });
    });
    // ── /help Command ─────────────────────────────────────────────────────────
    bot.onText(/\/help/, async (msg) => {
        const chatId = msg.chat.id.toString();
        await bot.sendMessage(chatId, `📖 *CopySignal Bot Commands*\n\n` +
            `/start — Link your CopySignal account\n` +
            `/status — Check bot connection status\n` +
            `/help — Show this help message\n\n` +
            `🌐 Dashboard: copysignal-bot.fundztechnology.com\n` +
            `💬 Support: @FundzTechnology`, { parse_mode: 'Markdown' });
    });
...
export async function sendPaymentAlert(telegramUserId, plan, chain) {
    if (!bot)
        return;
    const msg = `
💰 *Payment Confirmed!*
