...
export interface ParsedSignal {
  ...
  useMarketPrice: boolean;      // true when entry says "market" / "current price"
  take_profits: number[];
...
export function scoreSignal(parsed: ParsedSignal, channelRules?: ChannelRules): number {
  ...
  if (!parsed.entry && !parsed.useMarketPrice) return 0;
  if (!parsed.stop_loss) return 0;  // No stop loss = cannot trade safely
